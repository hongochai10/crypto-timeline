...
export default function RSAStation({ era }: StationProps) {
  ...
  const quizQuestions = getQuizQuestions("rsa", (key) => tq(key));
    ...
    <div className="flex flex-col gap-8">
      ...
      >
        ...
        <p className="text-sm leading-relaxed" style={{ color: "var(--text-secondary)" }}>
          {t.rich("narrative1", { em: (chunks) => <em>{chunks}</em> })}
        </p>
      ...
      <BenchmarkPanel algorithm="rsa" color={era.color} />
      ...
      <StationQuiz eraId="rsa" color={era.color} questions={quizQuestions} />
    </div>
  );
}
